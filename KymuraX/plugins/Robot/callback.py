...
from KymuraX import YouTube, app
from KymuraX.core.call import Kymura
from KymuraX.misc import SUDOERS, db
from KymuraX.utils.database import (is_active_chat,
                                       is_music_playing, set_loop,
                                       is_nonadmin_chat, music_off,
                                       music_on)
from KymuraX.utils.decorators.language import languageCB
from KymuraX.utils.formatters import seconds_to_min
from KymuraX.utils.inline.play import (panel_markup_1,
                                          stream_markup,
                                          telegram_markup)
from KymuraX.utils.stream.autoclear import auto_clean
from KymuraX.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Kymura.pause_stream(chat_id)
        ...
        await Kymura.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await Kymura.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Kymura.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Kymura.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Kymura.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
